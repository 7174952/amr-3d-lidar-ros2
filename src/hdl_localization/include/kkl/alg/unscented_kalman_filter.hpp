...
public:
  ...
  /**
   * @brief predict (without control input)
   */
  void predict() {
  ...
  /**
   * @brief correct with measurement update
  ...
  void correct(const VectorXt& measurement) {
    ...
    const auto& K_gain = kalman_gain;

    VectorXt ext_mean = ext_mean_pred + K_gain * (measurement - expected_measurement_mean);
    MatrixXt ext_cov = ext_cov_pred - K_gain * expected_measurement_cov * K_gain.transpose();
...
private:
   ...
   * @param sigma_points  output sigma points
   */
  ...
  /**
   * @brief Ensure covariance matrix is positive finite
   * @param cov  covariance matrix
   */
  void ensurePositiveFinite(MatrixXt& cov) {
    // 这里可以添加必要的数值稳定性处理，目前为空实现
    return;
